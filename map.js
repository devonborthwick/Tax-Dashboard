...
//Create a feature layer from your URL
  ...
  var laborForceUrl = "https://services.arcgis.com/P3ePLMYs2RVChkJx/ArcGIS/rest/services/ACS_10_14_Labor_Force_Participation_by_Age_Boundaries/FeatureServer/0"
  const laborForceLayer = new FeatureLayer({
    url: laborForceUrl,
...
r.features.forEach(function(feature){
 ...
 let additionalQuery1 = laborForceLayer.createQuery();
  additionalQuery1.returnGeometry = false;
  additionalQuery1.outFields = "*"
  ////execute query on your new layer
 laborForceLayer.queryFeatures(additionalQuery1).then(function(r){
console.log(r)
r.features.forEach(function(feature){
   ///loop through the results to add new features to your stateObj
  if(feature.attributes['STUSPS'] != "DC"){
      stateName = feature.attributes['NAME']
      var population = feature.attributes['B23027_001E']
    
      stateObj[stateName]['laborForce'] = population
  }
})
})
};
